...
to setup
  ...
  if buffer-battery? [set buf-bat-cap buffer-battery-capacity ]
...
NOTE: The current model is NOT the final model. For illustrative purposes only. Final model is expected Q1 2023. 
...
ENERGY-CONSUMPTION: The hourly energy consumption of an electric vehicle, expressed in %. 

CHARGING-SPEED. The hourly charging speed of the electric vehicles, expressed in %. 

MIN-CHARGE. The minimum required charge required for a vehicle to perform a job, expressed in % of the total battery capacity. If the current charge of a vehicle is below this threshold, it is not eligible to be assigned to a job. 
...
PARAM-ENERGY. Denotes the relative important of the ‘will’ of the battery of the electric vehicles when the ‘bee-berry’ algorithm is used. This parameters together with PARAM-ENERGY and PARAM-JOBS should sum up to 1. 
...
FREQUENCY. This parameters sets how often the algorithm is evaluated expressed in minutes. Default is 30 minutes. 
...
The current model is NOT the final model. Final model is expected Q1 2023. 
...
Default setting: decisions on allocating workers to jobs is done every 30 minutes (i.e., every 30 ticks). Can be controlled using the parameter FREQUENCY. 
